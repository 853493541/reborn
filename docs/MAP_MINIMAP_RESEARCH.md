...
## 12. Per-map bundle inventory (绝境 maps)

The three display UIs are shared/global (MiddleMap, MiniMap, BattleFieldMap),
so component coverage is **3/3 for every map**. Per-map data availability
(CDN index + local hpkg cache; descriptors extracted to
`proof/minimap/extracted/data/source/maps/`):

| map (display) | internal | tiles | middlemap | `[config]` | area.tab | npc.tab | extracted |
|---|---|---|---|---|---|---|---|
| 龙门绝境 | 龙门寻宝 | 1296 + 1296 (night) | 2048×1792 day+night | `128 / 0.02` | 18 zones | 0 B | **full** (tiles+all) |
| 沧溟绝境 | 海岛绝境 | 1296 | 2048×1792 | `128 / 0.02` | 2 rows | 0 B | descriptors |
| 白龙绝境 | 白龙绝境 | 1296 | 2048×1792 | `128 / 0.02` | 2 rows | 0 B | descriptors |
| 天原绝境 | 天原绝境 | 1296 | 2048×1792 | `128 / 0.02` | 2 rows | 0 B | descriptors |
| 洱海绝境 | 洱海绝境 | 1319 (+`doodad.tab`, `trafficline.tab`, `trafficnode.tab`) | 2048×1792 | `128 / 0.02` | 21 rows | 101 B | descriptors |
| 林海绝境 | 林海绝境 | **2704** (52×52) | 2048×1792 | `256 / 0.01`, offsets `-1024/-1024` | 2 rows | 111 B | descriptors |

Notes:
- 海岛绝境's `config.ini` carries `name=龙门寻宝` and the identical middlemap
  transform (`0.005867004`, `-21476.04`, `-10226.68`) — looks like an authoring
  copy-paste; verify its `middlemap.png` alignment before using.
- 林海绝境 proves the config is per-map: `width=256`, `scale=0.01`, offsets
  `-1024/-1024` → 25,600 world units/tile with an offset, 52×52 tiles.
- Tile extraction for any map uses the same CDN hpkg pipeline as 龙门寻宝
  (all packages are already `fullDownloaded` locally).
