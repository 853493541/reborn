// Map/scene loading recon for Spike B.
// Usage: dotnet fsi tools/dump_map_recon.fsx > engine_host_spike/recon_map.txt
...
let rec decodeType (md: MetadataReader) (r: BlobCursor) : string =
    let et = r.ReadByte()
    match et with
    | 0x01uy -> "void" | 0x02uy -> "bool" | 0x03uy -> "char" | 0x04uy -> "sbyte"
    | 0x05uy -> "byte" | 0x06uy -> "short" | 0x07uy -> "ushort" | 0x08uy -> "int"
    | 0x09uy -> "uint" | 0x0Auy -> "long" | 0x0Buy -> "ulong" | 0x0Cuy -> "float"
    | 0x0Duy -> "double" | 0x0Euy -> "string"
    | 0x0Fuy -> (decodeType md r) + "*"
    | 0x10uy -> (decodeType md r) + "&"
    | 0x11uy | 0x12uy ->
        let c = r.ReadCompressedInteger()
        typeNameOfDefOrRef md c
    | 0x13uy -> let i = r.ReadCompressedInteger() in sprintf "!%d" i
    | 0x14uy ->
    ...
    | 0x18uy -> "IntPtr" | 0x19uy -> "UIntPtr" | 0x1Buy -> "fnptr" | 0x1Cuy -> "object"
    | 0x1Duy -> (decodeType md r) + "[]"
    | 0x1Euy -> let i = r.ReadCompressedInteger() in sprintf "!!%d" i
    | 0x1Fuy | 0x20uy ->
    ...
    for th in md.TypeDefinitions do
        ...
        if md.GetString(t.Name) = tn then
            ...
            for mh in t.GetMethods() do
                ...
                printfn "   %s%s %s rva=%d" (if isStatic then "static " else "") (md.GetString m.Name) (methodSig md m) m.RelativeVirtualAddress
...
printfn "################ EXE IL DUMPS ################"
dumpIL exePath "MainForm" "LoadMap"
dumpIL exePath "EditorConfig" "get_EnableMapAysncLoad"
dumpIL exePath "EditorConfig" "get_EnableSceneFullLoading"
dumpIL exePath "EditorConfig" "get_EnableBufferReaderCache"
dumpIL exePath "SceneForm" "LoadScene"
dumpIL exePath "SceneForm" "OnBeforeLoadMap"
dumpIL exePath "SceneForm" "OnAfterLoadMap"
dumpIL exePath "SceneForm" "MoveCamera"
dumpIL exePath "SceneForm" "SceneExecAction"
dumpIL exePath "KPlayerCheckTool" "InitScene"
dumpIL exePath "EngineLayer" "NewScene"

printfn "################ REFS (exe) ################"
findRefs exePath [ "LoadMap"; "LoadEmptyScene"; "SetSceneFullLoading"; "EnableMapAsyncLoad"; "SwitchPlayerEnvironment"; "SetActiveEnvironment"; "GetLoadingProgress"; "ResetEnvironment" ]

printfn "################ TYPES ################"
dumpType exePath "MainForm"
dumpType clrPath "KMovieCore"
dumpType clrPath "KG_EnvironmentCLR"
dumpType clrPath "KGRC_CLR"

printfn "################ ENUMS ################"
listEnumsContaining clrPath "SCENE"
listEnumsContaining exePath "SCENE"
